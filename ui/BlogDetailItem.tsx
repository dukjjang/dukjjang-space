...
const BlogDetailItem = ({ post, nextPath, prevPath }: Props) => {
  return (
    <article
      id={post._id}
      className={` text-inherit relative h-full w-full grid grid-cols-[1fr_min(80ch,100%)_1fr] 
      auto-rows-max [&>div]:col-end-2 [&>div]:col-start-2 [&>div]:px-4
      overflow-x-hidden bg-background`}
    >
      {/* Main Image */}
      {post.mainImage && (
        <div className={`z-40 !col-start-1 !col-end-4 relative h-60 `}>
          <Img
            draggable="false"
